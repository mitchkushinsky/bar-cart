...
- generic_type: the class of bottles a bartender would treat as interchangeable WORKING SUBSTITUTES for this one. The test: if this bottle ran out, what other bottle would you reach for and still consider it basically the same drink? That's generic_type — not shared flavor origin, not the spreadsheet category, and not this bottle's own distinctive character. Lowercase, short (2-5 words). For products a recipe specifies by ROLE rather than by product name — bitters above all — apply that same substitutability test at the level of the role, not the specific flavor; see trap (2) below for exactly how that works and where its limits are.
  ...
  (2) Family resemblance is not substitutability — for most products. "Herbal" or "bitter" or "spiced" describes dozens of genuinely non-interchangeable products: Jägermeister, Strega, and Genepy are all loosely "herbal liqueurs" but are not working substitutes for each other (wildly different flavor, proof, and use), so each keeps its own type. This is the default, and it's still the rule for spirits and liqueurs without exception.

  Bitters are a deliberate exception, because of how a recipe actually calls for them: "2 dashes aromatic bitters" names a role, not a product, so the working-substitute test applies at the level of the role a bottle plays, not its specific flavor. Angostura, Peychaud's, Creole, Raft Aromatic, and other classic heavy-spiced aromatic bitters converge on one "aromatic bitters" type for exactly this reason — a recipe asking for aromatic bitters is satisfied by any of them. Group the rest of a bitters collection the same way, by the role a recipe would actually name: orange, chocolate/mole, nut, orchard and stone fruit, non-orange citrus, floral, vegetal/herbal, smoke/wood, chili/pepper, and umami/savory are each a genuine, separate role.

  Do not let this collapse further than that. A chocolate bitters, a nut bitters, a chili bitters, and an umami bitters are all loosely "specialty" but are not substitutes for each other or for aromatic — each is its own role, one type per role, not one type for all of them. And a role-based grouping never absorbs a product that plays a structurally different role just because it's also used a dash or drop at a time: a saline solution, a fruit shrub, and a bitters are three different structural roles in a drink, not three flavors of one catch-all bucket, no matter how broadly you're grouping bitters by role. Legitimate singletons — a product with no genuine sibling in either sense, role or flavor — remain legitimate; broadening toward role only applies where a real sibling exists.

  This role-based reasoning is specific to bitters and to other dash/drop products a recipe names by role rather than by product (a tincture used the same way, for instance). Spirits and liqueurs keep the product-based substitutability test above unchanged — "rye whiskey" still means the spirit, not a role a bottle plays.
